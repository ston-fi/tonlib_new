...
use crate::emulators::tx::tx_emul_args::{EmulArgs, EmulOrdinalArgs, EmulTickTockArgs};
...
impl TXEmulator {
    ...
    pub fn emulate_ordinal(&mut self, args: &EmulOrdinalArgs) -> Result<TXEmulationSuccess, TonlibError> {
    ...
    pub fn emulate_ticktock(&mut self, args: &EmulTickTockArgs) -> Result<TXEmulationSuccess, TonlibError> {
        self.prepare_emulator(&args.emul_args)?;
        let state_c_str = make_b64_c_str(&args.emul_args.shard_account_boc)?;
        let response_ptr = unsafe {
    ...
    fn prepare_emulator(&mut self, args: &EmulArgs) -> Result<(), TonlibError> {
...
mod tests {
    use super::*;
    use crate::emulators::tx::tx_emul_args::EmulArgs;
    ...
    fn test_tx_emulator_emulate_ext_in_wallet() -> anyhow::Result<()> {
        ...
        let mut ord_args = EmulOrdinalArgs {
        ...
        assert_err!(emulator.emulate_ordinal(&ord_args));
        ord_args.emul_args.ignore_chksig = true;
        let response = assert_ok!(emulator.emulate_ordinal(&ord_args));
