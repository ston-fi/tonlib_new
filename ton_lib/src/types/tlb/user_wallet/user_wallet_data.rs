...
pub struct UserWalletDataHLV2R2 {
    pub wallet_id: i32,
...
mod test {
    ...
    use crate::types::user_wallet::constants::DEFAULT_USER_WALLET_ID;
    use crate::types::user_wallet::constants::{DEFAULT_USER_WALLET_ID_V5R1, DEFAULT_USER_WALLET_ID_V5R1_TESTNET};
    ...
    fn test_wallet_data_v3() -> anyhow::Result<()> {
        ...
        let wallet_data = UserWalletDataV3::from_boc_hex(src_boc_hex)?;
        assert_eq!(wallet_data.seqno, 1);
        assert_eq!(wallet_data.wallet_id, DEFAULT_USER_WALLET_ID);
        assert_eq!(
        ...
        let restored = UserWalletDataV3::from_boc_hex(&serial_boc_hex)?;
    ...
    fn test_wallet_data_v4() -> anyhow::Result<()> {
        ...
        let wallet_data = UserWalletDataV4::from_boc_hex(src_boc_hex)?;
        ...
        let serial_boc_hex = wallet_data.to_boc_hex(false)?;
        let restored = UserWalletDataV4::from_boc_hex(&serial_boc_hex)?;
        assert_eq!(wallet_data, restored);
        Ok(())
    }
    ...
    fn test_wallet_data_v5() -> anyhow::Result<()> {
        ...
        let wallet_data = UserWalletDataV5::from_boc_hex(src_boc_hex)?;
        assert_eq!(wallet_data.seqno, 1);
        assert_eq!(wallet_data.wallet_id, DEFAULT_USER_WALLET_ID_V5R1);
        assert_eq!(
            wallet_data.public_key,
            TonHash::from_str("cbf377c9b73604c70bf73488ddceba14f763baef2ac70f68d1d6032a120149f4")?
        );
        ...
        let restored = UserWalletDataV5::from_boc_hex(&serial_boc_hex)?;
        assert_eq!(wallet_data, restored);
        Ok(())
    }
    ...
    fn test_wallet_data_v5_testnet() -> anyhow::Result<()> {
        ...
        assert_eq!(wallet_data.wallet_id, DEFAULT_USER_WALLET_ID_V5R1_TESTNET);
        Ok(())
    }
}
