...
pub(crate) fn all_bits_same(val: &BigUint) -> bool {
    if val.is_zero() {
        return true;
    }
...
pub(crate) fn common_prefix_len(a: &BigUint, b: &BigUint) -> usize {
    let xor = a ^ b;
    (a.bits() - xor.bits() - 1) as usize // don't forget leading zero
}

pub(crate) fn remove_leading_bit(val: &BigUint) -> BigUint {
    let bits = val.bits();
    let mask = BigUint::one() << (bits - 1);
    val ^ mask
}

pub(crate) fn add_leading_bit(val: &BigUint, val_bit_len: usize) -> BigUint {
